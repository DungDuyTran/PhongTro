...
function ChartTooltipContent(_a) {
    ...
    return (React.createElement("div", { className: utils_1.cn("border-border/50 bg-background grid min-w-[8rem] items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs shadow-xl", className) },
...
function ChartLegendContent(_a) {
    ...
    return (React.createElement("div", { className: utils_1.cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className) }, payload.map(function (item) {
        var key = "" + (nameKey || item.dataKey || "value");
        var itemConfig = getPayloadConfigFromPayload(config, item, key);
        return (React.createElement("div", { key: item.value, className: utils_1.cn("[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3") },
