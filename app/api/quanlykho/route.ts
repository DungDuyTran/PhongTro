...
const QuanLyKhoSchema = z.object({
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const data = await prisma.quanLyKho.findMany({
      skip: (page - 1) * limit,
      take: limit,
      include: {
        Kho: true,
...
export async function POST(req: NextRequest) {
  try {
    ...
    const success = QuanLyKhoSchema.safeParse(data);
    if (!success.success) {
      return NextResponse.json(
    ...
    const newData = await prisma.quanLyKho.create({
      data: {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const update = await prisma.quanLyKho.updateMany({
      data: {
        ngayBatDau: success.data.ngayBatDau,
        ngayKetThuc: success.data.ngayKetThuc,
        KhoId: success.data.KhoId,
...
export async function DELETE(req: NextRequest) {
  try {
    const deleted = await prisma.quanLyKho.deleteMany();
