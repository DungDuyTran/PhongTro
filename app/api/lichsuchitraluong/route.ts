...
// {
//     "ghiChu": "Phat luong thang 4",
//     "ChiTraLuongId": 1
//   }

export async function GET(req: NextRequest) {
  try {
    const data = await prisma.lichSuChiTraLuong.findMany({
      include: {
        ChiTraLuong: true,
      },
    });
...
export async function POST(req: NextRequest) {
  try {
    ...
    const newData = await prisma.lichSuChiTraLuong.create({
      data: {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    const update = await prisma.lichSuChiTraLuong.updateMany({
      data: {
        ghiChu: success.data.ghiChu,
        ChiTraLuongId: success.data.ChiTraLuongId,
      },
